...
if( ! function_exists( 'best_learner_site_branding' ) ) :
	/**
  	 * Site Branding
  	 *
  	 * @since 1.0.0
  	 */
function best_learner_site_branding() { ?>
    <div class="wrapper">
        ...
        <nav id="site-navigation" class="main-navigation" role="navigation" aria-label="Primary Menu">
            ...
            wp_nav_menu( array(
                ...
                'menu_class'     => 'nav-menu',
                'fallback_cb'    => 'best_learner_primary_navigation_fallback',
            ) );
            ?>
        </nav><!-- #site-navigation -->
    </div><!-- .wrapper -->
...
add_action( 'best_learner_action_header', 'best_learner_site_branding', 10 );

if ( ! function_exists( 'best_learner_footer_top_section' ) ) :
   ...
   * @since 1.0.0
   */
  function best_learner_footer_top_section() {
      $footer_sidebar_data = best_learner_footer_sidebar_class();
...
add_action( 'best_learner_action_footer', 'best_learner_footer_top_section', 10 );

if ( ! function_exists( 'best_learner_footer_section' ) ) :
  ...
  function best_learner_footer_section() { ?>
        <div class="site-info">
            <?php 
                $copyright_footer = best_learner_get_option('copyright_text'); 
                if ( ! empty( $copyright_footer ) ) {
                $copyright_footer = wp_kses_data( $copyright_footer );
                }
                // Powered by content.
                $powered_by_text = sprintf( __( ' Theme Best Learner by %s', 'best-learner' ), '<a target="_blank" rel="designer" href="http://creativthemes.com/">Creativ Themes</a>' );
            ?>
            <div class="wrapper">
...
add_action( 'best_learner_action_footer', 'best_learner_footer_section', 20 );

if ( ! function_exists( 'best_learner_footer_sidebar_class' ) ) :
  /**
   * Count the number of footer sidebars to enable dynamic classes for the footer
   *
   * @since Best Learner 0.1
   */
  function best_learner_footer_sidebar_class() {
    $data = array();
...
if ( ! function_exists( 'best_learner_excerpt_length' ) ) :
   ...
   */
  function best_learner_excerpt_length( $length ) {

    if ( is_admin() ) {
      return $length;
    }

    $excerpt_length = best_learner_get_option( 'excerpt_length' );
...
function best_learner_banner_title(){ 
    ...
    if( is_search() ){ ?>
        <h2 class="page-title"><?php printf( esc_html__( 'Search Results for: %s', 'best-learner' ), '<span>' . get_search_query() . '</span>' ); ?></h2>
    <?php }
    
    if( is_404() ) {
        echo '<h2 class="page-title">' . esc_html__( 'Error 404', 'best-learner' ) . '</h2>';
    }
}
endif;
 ...
 * @since 1.0
 */
function best_learner_render_social_links() {

        $social_link1 = best_learner_get_option( 'social_link_1' );
        $social_link2 = best_learner_get_option( 'social_link_2' );
        $social_link3 = best_learner_get_option( 'social_link_3' );
